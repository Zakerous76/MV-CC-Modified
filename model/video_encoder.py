...
class Video_encoder(nn.Module):
    ...
    def __init__(self, vision_encoder_pretrain='checkpoints/vision_encoder.pth', if_lora=True):
        ...
        # Force the model to load on CUDA explicitly by passing map_location:

        # print("torch.cuda.is_available():", torch.cuda.is_available())
        # print("device count:", torch.cuda.device_count())
        # print("device name:", torch.cuda.get_device_name(0) if torch.cuda.device_count() > 0 else "N/A")

        self.model = torch.load(vision_encoder_pretrain, map_location=torch.device('cuda'), weights_only=False)
