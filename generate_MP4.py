"""
Generates an interpolated video from each image pair in LEVIR_CC dataset, (2 fps ⇒ 8 frames) with OpenCV.
The outputs are saved in ./LEVIR-MCI-dataset/images/{dataset}/video_data
"""
import cv2
import os
...
for dataset in datasets:
    img_directory = f'./LEVIR-MCI-dataset/images/{dataset}'
    file_names = os.listdir(f"{img_directory}/A")
    ...
    for file_name in tqdm(file_names):
        ...
        out = cv2.VideoWriter(f'{video_dir}/{name_without_extension}.mp4', 
                              fourcc,                             # video format
                              2.0,                                # fps
                              (image1.shape[1], image1.shape[0])) # resolutions
        for i in range(8):
            weight = i / 7.0  
    ...
    # zakerous76: If I wanted to increase the size of the video
        # fps = 2.0
